...
class FLUID(nn.Module):
    ...
    def zero_shot_inpaint(self, init_tokens, inpaint_mask, num_iter=64, cfg=1.0, cfg_schedule="linear",
                      texts=None, temperature=1.0, height=512, width=512, progress=False):
        assert height % (self.vae_stride * self.patch_size) == 0 and width % (self.vae_stride * self.patch_size) == 0
        self.seq_h, self.seq_w = height // self.vae_stride // self.patch_size, width // self.vae_stride // self.patch_size
        self.seq_len = self.seq_h * self.seq_w
        bsz, seq_len, _ = init_tokens.shape
        
        assert seq_len == self.seq_len
        tokens = init_tokens.clone()  
       
        gen_mask = inpaint_mask.float().cuda() 
        input_token = torch.ones(1, seq_len).bool()
        
        orders = self.sample_orders(bsz).cuda()
        # orders = orders * inpaint_mask + (1 - inpaint_mask) * 1e6
        orders = orders[torch.isin(orders, torch.nonzero(inpaint_mask, as_tuple=True)[1])].view(bsz, -1)
        # orders = orders.masked_select((1-inpaint_mask).bool()).view(bsz, -1)
        # orders_filtered = torch.stack([
        #     order[torch.isin(order, torch.nonzero(inpaint_mask[i], as_tuple=True)[1])]
        ...
        for step in indices:
            ...
            if cfg != 1.0:
                tokens_ = torch.cat([tokens, tokens], dim=0)
                text_embeddings_ = torch.cat([text_embeddings, self.fake_latent.expand(bsz, self.max_length, self.cross_embed_dim)], dim=0)
                mask_ = torch.cat([gen_mask, gen_mask], dim=0)
            else:
                tokens_ = tokens
                text_embeddings_ = text_embeddings
                mask_ = gen_mask

            x = self.forward_fluid_encoder(tokens_, mask_)
            x = self.forward_fluid_decoder(x, mask_)
            z = self.forward_fluid_text_decoder(x, text_embeddings_)

            total_inpaint = gen_mask.sum(dim=1, keepdim=True)
            mask_ratio = np.cos(math.pi / 2. * (step + 1) / num_iter)
            # mask_len = torch.Tensor([np.floor(total_inpaint * mask_ratio)]).cuda()
            mask_len = torch.floor(total_inpaint * mask_ratio).cuda()
            mask_len = torch.maximum(torch.Tensor([1]).cuda(),
                                    torch.minimum(total_inpaint - 1, mask_len))
            
            mask_next = mask_by_order(mask_len[0], orders, bsz, seq_len)
            if step >= 34:
                mask_to_pred = gen_mask.bool()
            else:
                mask_to_pred = torch.logical_xor(gen_mask.bool(), mask_next.bool())
            gen_mask = mask_next

            if cfg != 1.0:
                mask_to_pred = torch.cat([mask_to_pred, mask_to_pred], dim=0)
                input_index = torch.cat([input_token, input_token], dim=0)

            # z_pred = z[mask_to_pred.nonzero(as_tuple=True)]
            z_input = z[input_index.nonzero(as_tuple=True)]
            if cfg_schedule == "linear":
                cfg_iter = 1 + (cfg - 1) * (total_inpaint - mask_len[0]) / total_inpaint
            elif cfg_schedule == "constant":
                cfg_iter = cfg
            else:
                raise NotImplementedError("Unsupported CFG schedule")
            # sampled_token_latent = self.diffloss.sample(z_pred, temperature, cfg_iter)
            sampled_token_latent = self.diffloss.sample(z_input, temperature, cfg_iter)
            # sampled_token_latent = sampled_token_latent[mask_to_pred.nonzero(as_tuple=True)]
            if cfg != 1.0:
                sampled_token_latent, _ = sampled_token_latent.chunk(2, dim=0)
                mask_to_pred, _ = mask_to_pred.chunk(2, dim=0)

            cur_tokens[mask_to_pred.nonzero(as_tuple=True)] = sampled_token_latent.unsqueeze(0)[mask_to_pred.nonzero(as_tuple=True)]
            tokens = cur_tokens.clone()
            if step >= 34:
                break
